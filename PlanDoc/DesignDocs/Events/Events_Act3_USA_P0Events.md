...
**触发条件**：必须身份为`undocumented`，且移民执法压力强度≥2
...
interface IceRaidEvent {
  ...
  trigger: {
    ...
    minPressureLevel: 2;              // 压力强度≥2才会触发
    baseProbability: 0.05;
    probabilityModifiers: {
      perPressureLevel: 0.02;         // 每级压力+2%
      homelessBonus: 0.05;            // 露宿街头+5%
      overstayBonus: 0.05;            // 逾期滞留+5%
    };
  };
